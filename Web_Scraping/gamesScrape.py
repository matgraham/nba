...
#Set sys file for other module
import sys
sys.path.insert(0, r'F:\Projects\NBA\Preprocessing_Data')
...
#The second number in the range category should match the last number in the nba stats url
for i in range(124,329):
     if i < 9:
         time.sleep(1)
         webpull('http://stats.nba.com/game/002180000{}/'.format(i + 1))
         game_counter += 1
     elif i >= 999:
         time.sleep(.5)
         webpull('http://stats.nba.com/game/002180{}/'.format(i + 1))
         game_counter += 1
     elif i >= 99:
         time.sleep(.5)
         try: 
            webpull('http://stats.nba.com/game/0021800{}/'.format(i + 1))
            game_counter += 1
         except:
            continue
     else:
         time.sleep(1)
         webpull('http://stats.nba.com/game/00218000{}/'.format(i + 1))
         game_counter += 1
